...
func _ready(): #should go to start game UI
...
func enter_battle(formation: Formation): #formation = # of badguys
...
#	remove_child(local_map)
	...
	combat_arena.connect("battle_ends", self, "_on_CombatArena_battle_ends")
...
func _on_CombatArena_battle_ends(battlers):
	#todo call party function that updates stats
	for battler in battlers:
		print(battler.get_name() + "end battle text")
		for member in party.get_children():
			if member.get_name() == battler.get_name():
				member.battler.stats = battler.stats
